...
impl<T> DynamicNameGetter for T where T: StaticNameGetter + Sized {
    fn get_name_inst(&self) -> &'static str {
        Self::get_name()
    }
}
...
pub fn register_dialog<T>() where T : 'static + Dialog {
    let mut deserializers = DESERIALIZERS.lock().unwrap();
    deserializers.insert(T::get_name(), Box::new(|string: &str| {
        Box::new(T::from_string(string))
    }));
}
...
impl Serialize for Box<Dialog> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        ...
        seq_serializer.serialize_element(self.deref().get_name_inst())?;
        seq_serializer.serialize_element(self.deref().to_string().as_str())?;
        seq_serializer.end()
    }
}
