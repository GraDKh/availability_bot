...
impl SimpleDialog for HelpDialog {
    fn process_message(message:&str, user_info: &mut UserInfo) -> Option<(Option<ReplyMessage>, Option<Event>)> {
        if message.starts_with("/help") {
            Some((Some(ReplyMessage::new("https://www.youtube.com/watch?v=yWP6Qki8mWc", None)), None))
        }
        else {
            None
        }
...
impl SimpleDialog for SetMyNameDialog {
    fn process_message(message:&str, user_info: &mut UserInfo) -> Option<(Option<ReplyMessage>, Option<Event>)> {
        if message.starts_with(SET_MY_NAME_PREFIX) {
            let reply_message = match parse_name(message) {
                Some(name) => {
